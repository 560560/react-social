    ...
    <div className={s.usersBlock} key={u.id}>
      <div className={s.userAva_followButton}>
        ...
        <div className={s.followUnfollowButton}>{
          u.followed
            ? <button onClick={() => {
              unFollow(u.id).then(response => {if (response.data.resultCode === 0) {props.unfollowFrom(u.id)}})
            }}>Unfollow</button>
            : <button onClick={() => {
              follow(u.id).then(response => {if (response.data.resultCode === 0) {props.followTo(u.id)}})
            }}>Follow</button>}
        </div>
      </div>
